...
export function parseWhatsAppMessage(message) {
  ...
  const funnyTriggers = ["😂", ":)", "haha"];
  const loveTriggers = ["❤", "love", "pyaar"];

  const isFunny = funnyTriggers.some((k) =>
    lowerText.includes(k.toLowerCase()),
  );
  const isLove = loveTriggers.some((k) => lowerText.includes(k.toLowerCase()));
